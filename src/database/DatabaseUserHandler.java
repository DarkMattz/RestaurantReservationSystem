...
public class DatabaseUserHandler {
    ...
    public boolean checkEmail(String email) {
        ResultSet res = con.executeQuery("SELECT * FROM users WHERE email = '" + email + "'");
        try {
            return res.next();
    ...
    public Customer findCustomer(String email, String password) {
        ResultSet res = con.executeQuery("SELECT * FROM `users` WHERE email = '" + email + "' AND password = '" + password +"'");
        try {
            if(res.next()){
    ...
    public Admin findAdmin(String email, String password) {
        ResultSet res = con.executeQuery("SELECT * FROM `users` WHERE email = '" + email + "' AND password = '" + password +"' AND isAdmin = " + "1");
        try {
            if(res.next()){
    ...
    public void editName(Customer customer, String name) {
        con.executeUpdate("UPDATE `users` SET `name`='"+ name +"' WHERE `email` = '" + customer.getEmail() + "'");
    }

    public void editPhone(Customer customer, String phone) {
        con.executeUpdate("UPDATE `users` SET `phone`='"+ phone +"' WHERE `email` = '" + customer.getEmail() + "'");
    }

    public void editPass(Customer customer, String pass) {
        con.executeUpdate("UPDATE `users` SET `password`='"+ pass +"' WHERE `email` = '" + customer.getEmail() + "'");
    }
